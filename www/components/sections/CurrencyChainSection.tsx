...
export function CurrencyChainSection({
  ...
  useEffect(() => {
    ...
    const defaultToken = tokens.find((t) => t.address === recipientTokenAddress) || tokens[0];
    if (defaultToken) {
          ...
          <div className="space-y-2">
            {tokens.map((token) => (
              <button
                ...
                style={{
                  background: selectedToken?.address === token.address && !useCustomToken
                    ? 'var(--color-muted)'
                    : 'var(--color-background)',
                  border: selectedToken?.address === token.address && !useCustomToken
                ...
                onMouseEnter={(e) => {
                  if (selectedToken?.address !== token.address || useCustomToken) {
              ...
              {useCustomToken && (
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value && /^0x[a-fA-F0-9]{40}$/.test(e.target.value)) {
